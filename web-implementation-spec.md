...
- **Constructs not modeled by the parser** (§4), and therefore invisible to `validateSchema` (§16) even though they're common in large real-world government schemas:
  ...
  - `xs:redefine`
  - ~~Complex-type inheritance via `xs:extension` on `xs:complexContent`~~ — **resolved**: the real Hawaii N-11 schema set leans on this heavily (every MeF-style leaf form is `<complexContent><extension base="FormXxxType">`, with the base complexType holding every field and the extension itself adding only an `xs:attributeGroup ref`). `SchemaParser._resolveEffectiveParticles`/`_resolveEffectiveAttributeEls` (§4) now follow the base-type chain (and `xs:attributeGroup` refs) recursively when building a `GroupContainer`'s children — without this, an extension-based form rendered with zero fields. `xs:restriction` on `xs:complexContent` remains unmerged (own-particles-only) — not exercised by any schema seen so far.
